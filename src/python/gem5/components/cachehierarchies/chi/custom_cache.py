...
from .nodes.private_l1_mesi_cache import PrivateL1MESICache
...
class CustomCHICacheHierarchy(AbstractRubyCacheHierarchy):
    """A custom multi-level CHI cache hierarchy

    This hierarchy supports:
    - Private L1I/L1D caches per core
    - Private L2 caches per core
    - Shared L3 caches (distributed)
    ...
    def incorporate_cache(self, board: AbstractBoard) -> None:
        ...
        self.core_clustersHCPU = [
            self._create_core_cluster(core, i, board, False)
            for i, core in enumerate(board.get_processor().get_cores())
            if i < 17  # Stop the list comprehension at 17
        ]
        # Create core clusters (L1)
        self.core_clusterNDA = [
            self._create_core_cluster(core, i, board, True)
            for i, core in enumerate(board.get_processor().get_cores())
            if i >= 17  # Start the list comprehension at 17
        ]
        ...
        # Count sequencers
        num_sequencers = (
            len(self.core_clustersHCPU) * 3
        )  # L1I + L1D + L2 per core
        num_sequencers += len(self.core_clusterNDA) * 2  # L1I + L1D per core
        if board.has_dma_ports():
    ...
    def _create_core_cluster(
        ...
        if different_core:
            ...
            cluster.icache = PrivateL1MESICache(
                size=self._l1i_size,
            ...
            cluster.dcache = PrivateL1MESICache(
                size=self._l1d_size,
